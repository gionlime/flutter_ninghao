 ...
 * Author: Jeek
 * Date: 2020/12/14
 * Description: Wraps NSUserDefaults (on iOS) and SharedPreferences (on Android), providing
 */
class SPUtils {

  SPUtils._(this._preferenceCache);

  static const String _prefix = 'flutter.';
  static Completer<SPUtils> _completer;
  ...
  static Future<SPUtils> getInstance() async {
    if (_completer == null) {
      _completer = Completer<SPUtils>();
      try {
        final Map<String, Object> preferencesMap =
        await _getSharedPreferencesMap();
        _completer.complete(SPUtils._(preferencesMap));
      } on Exception catch (e) {
        // If there's an error, explicitly return the future with an error.
        // then set the completer to null so we can retry.
        _completer.completeError(e);
        final Future<SPUtils> sharedPrefsFuture = _completer.future;
        _completer = null;
  ...
  Future<void> reload() async {
    ...
    await SPUtils._getSharedPreferencesMap();
    _preferenceCache.clear();
